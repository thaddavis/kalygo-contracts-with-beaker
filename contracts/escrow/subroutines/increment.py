...
class Subroutine_Increment_Mixin(Application):
    ...
    global_buyer_pullout_flag: ApplicationStateValue

    # @external(authorize=Authorize.only(Global.creator_address()))
    @external()
    def increment(self):
        return Seq(
            If(self.global_buyer_pullout_flag + Int(1) < Int(10))
            .Then(
                self.global_buyer_pullout_flag.set(
                    self.global_buyer_pullout_flag + Int(1)
                ),
                Approve(),
            )
            .Else(Reject())
